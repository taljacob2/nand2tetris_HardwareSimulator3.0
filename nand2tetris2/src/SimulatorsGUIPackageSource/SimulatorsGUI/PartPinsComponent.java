...
public class PartPinsComponent extends PinsComponent implements PartPinsGUI {
    ...
    //TODO: SETS THE COLORS OF THE 'Part pins' TABLE !!!
    ...
    class PartPinsTableCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent
            ...
            if (column == 0 || column == 1) {
                setHorizontalAlignment(SwingConstants.CENTER);//! TODO: this is how you center the Text in the column !
                setForeground(Colors.getInstance().partPinsTable_textForeground);//TODO: Sets the color of the Table Text FOREGROUND
                setBackground(Colors.getInstance().partPinsTable_Partpin_and_Gatepin_ColumnsBackgroundRow);//TODO: Sets the color of the 'Part pin' & 'Gate pin' columns BACKGROUND
            }
            else {
                setHorizontalAlignment(SwingConstants.RIGHT);
                for (int i = 0; i < highlightIndex.size(); i++) {
                    if (row == ((Integer) highlightIndex.elementAt(i)).intValue()) {
                        setForeground(Colors.getInstance().markedDigits);//TODO: DOESN'T DO ANYTHING
                        break;
                    }
                    else
                        setForeground(null);//TODO: DON'T KNOW WHAT THIS IS
                }
                if (row == flashIndex) {
                    setBackground(Color.orange);//TODO: DON'T KNOW WHAT THIS IS
                }
                else
                    setBackground(Colors.getInstance().partPinsTable_Value_ColumnBackgroundRow);//TODO: Sets the color of the 'Value' column BACKGROUND
    ...
    private void jbInit() {
        ...
        //TODO: Part Pins table CHIP LABEL:::
        partNameLbl.setForeground(Colors.partPins_ChipLabel);

        if (ResolutionUtils.getInstance().CURRENT_SCALE == ResolutionUtils.Scale._3) {
            partNameLbl.setBounds(new Rectangle(0, 0, 156, ResolutionUtils.getInstance().ROW_HEIGHT));//TODO: SETS THE LOCATION / JLabel OF THE TEXT 'XOR' / 'And' /'Or' ... 'Part pins' table
        }
        else if (ResolutionUtils.getInstance().CURRENT_SCALE == ResolutionUtils.Scale._2) {
            partNameLbl.setBounds(new Rectangle(0, 0, 95, ResolutionUtils.getInstance().ROW_HEIGHT));//TODO: SETS THE LOCATION / JLabel OF THE TEXT 'XOR' / 'And' /'Or' ... 'Part pins' table
        }
        else if (ResolutionUtils.getInstance().CURRENT_SCALE == ResolutionUtils.Scale._1) {
            partNameLbl.setBounds(new Rectangle(0, 0, 62, ResolutionUtils.getInstance().ROW_HEIGHT));//TODO: SETS THE LOCATION / JLabel OF THE TEXT 'XOR' / 'And' /'Or' ... 'Part pins' table
        }
